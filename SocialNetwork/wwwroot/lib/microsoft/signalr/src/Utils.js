        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 2:
                    ...
                    headers[name] = value;
                    logger.log(ILogger_1.LogLevel.Trace, "(".concat(transportName, " transport) sending data. ").concat(getDataDetail(content, logMessageContent), "."));
        ...
        if (logLevel >= this.minimumLogLevel) {
            switch (logLevel) {
                ...
                case ILogger_1.LogLevel.Error:
                    this.outputConsole.error("[".concat(new Date().toISOString(), "] ").concat(ILogger_1.LogLevel[logLevel], ": ").concat(message));
                    break;
                case ILogger_1.LogLevel.Warning:
                    this.outputConsole.warn("[".concat(new Date().toISOString(), "] ").concat(ILogger_1.LogLevel[logLevel], ": ").concat(message));
                    break;
                case ILogger_1.LogLevel.Information:
                    this.outputConsole.info("[".concat(new Date().toISOString(), "] ").concat(ILogger_1.LogLevel[logLevel], ": ").concat(message));
                    break;
                default:
                    // console.debug only goes to attached debuggers in Node, so we use console.log for Trace and Debug
                    this.outputConsole.log("[".concat(new Date().toISOString(), "] ").concat(ILogger_1.LogLevel[logLevel], ": ").concat(message));
                    break;
            }
        }
    };
